...
var buildOldFormatCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if err = builder.UpdateFormatVersion(b, buildFlags.format); err != nil {
			fail(err)
		}
		setWorkers(b)
		...
		if err = builder.CopyFullGroupsINI(b); err != nil {
			fail(err)
		}
		...
		if err = builder.CopyTrimmedGroupsINI(b); err != nil {
			fail(err)
		}
		...
		if err = builder.RevertFullGroupsINI(b); err != nil {
			fail(err)
		}
		...
		prevFormat--
		if err = builder.UpdateFormatVersion(b, strconv.Itoa(prevFormat)); err != nil {
			fail(err)
		}
		...
		if err = builder.RevertTrimmedGroupsINI(b); err != nil {
			fail(err)
		}
